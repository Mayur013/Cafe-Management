...
public class ProductDao {
    ...
    public static void save(Product product){
        ...
        DbOperations.setDataOrDelete(query, "Product addedd Successfully!!!!");
    }
    ...
    public static ArrayList<Product> getAllRecordsByCategory(String catagory){
        ArrayList<Product> arrayList = new ArrayList<>();
        try{
            ResultSet rs=DbOperations.getData("select * from product where catagory ='"+catagory+"'");
            while(rs.next()){
                Product product=new Product();
                product.setName(rs.getString("name"));
                arrayList.add(product);
    ...
    public static ArrayList<Product> filterProductByName(String name,String catagory){
        ArrayList<Product> arrayList = new ArrayList<>();
        try{
            ResultSet rs=DbOperations.getData("select * from product where name like '%"+name+"%' and catagory ='"+catagory+"'");
            while(rs.next()){
                Product product=new Product();
                product.setName(rs.getString("name"));
                arrayList.add(product);
    ...
    public static Product getProductByName(String name){
        ...
        try{
            ...
            while(rs.next()){
                ...
                product.setCatagory(rs.getString(3));
            }
        }
        catch(Exception e){
            JOptionPane.showMessageDialog(null, e);
